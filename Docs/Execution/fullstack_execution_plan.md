...
1.  **Authentication & Profile:** Đăng ký, đăng nhập JWT, Email OTP (Gmail), quản lý profile, mảng kỹ năng và counter `joined_activity_count`.
...
1.  **SCR-01: Welcome / Login Screen:** Đăng nhập bằng email/password, chuyển hướng quên mật khẩu, nút sang Đăng ký.
2.  **SCR-02: Register Screen:** Nhập email, số điện thoại, mật khẩu, họ tên.
3.  **SCR-03: OTP Verification Screen:** Nhập mã OTP 6 số nhận được qua email để kích hoạt tài khoản.
...
```
[SCR-02: Đăng ký] ──► (POST /auth/register) ──► Tạo user ở trạng thái unverified (PENDING_OTP)
                                                         │
                                                         ▼
[SCR-03: Nhập OTP] ◄─────────────────────────── Gửi Email OTP qua Gmail
        │
        ▼ (POST /auth/verify-otp)
Kích hoạt thành công ──► Chuyển sang trạng thái ACTIVE ──► [SCR-01: Đăng nhập]
```
